...
public class EntelPeruAppTestVerifyPostPaidBalance implements ITest {
    ...
    static {
        STEPS_TEST = Collections.singletonMap("Step01", new AbstractMap.SimpleEntry<>("DST-ENTEL_PERU-D001-E001", "Acceder a Saldos"));
    }

    public EntelPeruAppTestVerifyPostPaidBalance(String phoneNumber, String verificationCode) {
    ...
    public TestResult runTest(AndroidDriver<AndroidElement> driver) throws InterruptedException {
        ...
        verifyBalance(driver, contentPath);
        return testResult;
    }

    public void verifyBalance(AndroidDriver<AndroidElement> driver, String contentPath) throws InterruptedException {
        ...
        testResult.setCode(EnumCodeProcessTests.VERIFY_POST_PAID_BALANCE_SUCCESS_ENTEL_PERU_APP.getCode());
        testResult.setMessage(EnumCodeProcessTests.VERIFY_POST_PAID_BALANCE_SUCCESS_ENTEL_PERU_APP.getDescription());
        long totalTimeOutAdd = System.currentTimeMillis(), currentTimeOut = System.currentTimeMillis();
        try {
            WebDriverWait wait = new WebDriverWait(driver, TestUtils.getWaitTimeout());
            String balanceOptionPath = contentPath + "//*[@resource-id='b4-b1-b1-BottomBarItems']/*[@class='android.view.View'][2]";
            wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(balanceOptionPath)))
                    .click();


            String balanceTextPath = contentPath + "//*[@resource-id='b1-b4-b10-Content']/*[@class='android.widget.TextView'][1]";
            String balanceTextLabel = wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(balanceTextPath)))
                    .getText();
            boolean expectedTextFound = balanceTextLabel.trim()
                    .equals("Datos de tu plan");

            String finalMessage = String.format("Texto \"Datos en tu plan\" %s fue encontrado en el TAB seleccionado.", (expectedTextFound ? "si" : "no"));
            ...
            testResult.addItemDetail((new TestResultDetailEntelApp().setCode("DST-ENTEL_PERU-D001-001")
