...
public class EntelPeruAppTestVerifyRenewal implements ITest {
    ...
    static {
        STEPS_TEST = Collections.singletonMap("Step01", new AbstractMap.SimpleEntry<>("DST-ENTEL_PERU-D009-E001", "Acceder a renovación"));
    }
    ...
    public void verifyRenewal(AndroidDriver<AndroidElement> driver, String contentPath) throws InterruptedException {
        ...
        try {
            ...
            String devicesOfferListFirstItemPath = contentPath + "//*[@resource-id='b1-b5-b1-Equipos']//*[@class='android.widget.Button']";
            String deviceOfferFirstItemText = wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(devicesOfferListFirstItemPath)))
                    .getText();
            boolean expectedTextFound = deviceOfferFirstItemText.trim()
                    .contains("LO QUIERO");
            String finalMessage = String.format("Texto \"LO QUIERO\" %s fue encontrado en el primer item del listado de dispositivo habilitados para renovacion.", (expectedTextFound ? "si" : "no"));
